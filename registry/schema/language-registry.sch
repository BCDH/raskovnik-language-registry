...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt">
  ...
  <pattern id="node-identity-and-shape">
    <rule context="tei:language | tei:languageGrp">
      ...
      <assert test="count(tei:name[@type = 'languageName'][@role = 'languageReferenceName']) = 3">Every node requires exactly three preferred names.</assert>
      <assert test="count(tei:name[@type = 'languageName'][@role = 'languageReferenceName'][@xml:lang = 'sr']) = 1 and count(tei:name[@type = 'languageName'][@role = 'languageReferenceName'][@xml:lang = 'en']) = 1 and count(tei:name[@type = 'languageName'][@role = 'languageReferenceName'][@xml:lang = 'de']) = 1">Preferred names must contain exactly one Serbian, English, and German value.</assert>
